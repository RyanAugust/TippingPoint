...
def run_dashboard():
  ...
  if max_spend:
    ...
    st.info(f"💡 **Strategic Recommendation:** You should spend at least **${min_spend:,.2f}** per day to exit the inefficient warm-up phase (Peak Efficiency). \
            However, do not scale spend beyond **${max_spend:,.2f}** per day (or **${annual_spend_m:.2f}M** annualized), as any additional dollar spent beyond this \
            threshold will return less than your target mROAS of **{target_mroas:.2f}**.")
  else:
