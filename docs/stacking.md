...
dxpoint is designed around three distinct, modular analytical pathways that can be stacked progressively based on business needs, channel scope, and empirical data availability:
...
3. **Pathway 3: Incrementality & Causal Calibration Stream**: A parallel validation stream where experimental data (conversion lift studies, holdout tests, geo-experiments) can be introduced at any point to validate or calibrate single-channel curves, joint multi-channel models, or portfolio allocations.
...
graph TD
    ...
    subgraph MultiPathway["Pathway 2: Multi-Channel & Portfolio Strategy (Extends from Multiple Single Channels)"]
        direction TB
        M1["2A. Multi-Channel Synergy & Attribution<br/>MultiChannelModel Joint Estimation<br/>Historical Attribution & Curve-Shifting Synergy"]
    ...
    subgraph CausalPathway["Pathway 3: Incrementality & Causal Lift Stream (Parallel Validation & Calibration)"]
        ...
        E2["Multi-Channel Parallel Calibration<br/>multi_model.attach_experiments(dict_or_list)<br/>Joint Bayesian Likelihood Calibration across Channels"]
...
* **Component**: [`MarketingReturnCurve`](../src/dxpoint/models.py) fitting the Hill response function:
...
* **Component**: Vectorized geometric ($\theta$) or Weibull ($k, \lambda$) adstock transformations in [`src/dxpoint/math.py`](../src/dxpoint/math.py).
* **Strategic Outputs**:
...
* **Component**: Goodness-of-fit suite in [`src/dxpoint/evaluation.py`](../src/dxpoint/evaluation.py) and Frequentist Delta-Method in [`src/dxpoint/models.py`](../src/dxpoint/models.py).
* **Strategic Outputs**:
...
* **Component**: [`MultiChannelModel`](../src/dxpoint/multichannel.py) joint estimation:
...
* **Component**: [`PortfolioAllocator`](../src/dxpoint/portfolio.py) constrained non-linear solver.
* **Strategic Outputs**:
...
```
  ...
  model.attach_experiments() multi_model.attach_experiments() allocator.validate_experiments()
  model.validate_experiments()multi_model.validate_experiments()allocator.get_calibration_summary()
```
...
* **Parallel Experiment Association**: Attach experiments across multiple channels simultaneously using channel-mapped dictionaries (`multi_model.attach_experiments({"YouTube": [exp1], "Paid Search": [exp2]})`).
...
| :--- | :--- | :--- |
| **Observational First** | Single Channel $\to$ Multi-Channel Model $\to$ Portfolio Allocator $\to$ Validate with Lift Tests Later | Teams with historical observational data who want immediate cross-channel budget optimization before executing formal lift tests. |
...
| **Selective Hybrid Calibration** | Multi-Channel Model with Lift Tests on Core Channels (e.g., YouTube, Paid Search) + Observational Priors on Exploratory Channels | Enterprise marketers with holdout studies on high-spend core channels operating alongside newer, uncalibrated media channels. |
